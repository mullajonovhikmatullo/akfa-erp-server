...
exports.SalesService = {
    ...
    async create(dto, user, idempotencyKey) {
        ...
        return prisma_1.prisma.$transaction(async (tx) => {
            ...
            if (claim?.replay) {
                const previous = await sales_repository_1.SalesRepository.findById(claim.resourceIds[0], storeId, tx);
                if (!previous || previous.branch.id !== branchId)
                    throw new AppError_1.AppError(409, "Original sale is unavailable");
                return previous;
            }
            // ── Validate branch ──────────────────────────────────────────────────
            ...
            await inventory_service_1.InventoryService.deductStockBatch(storeId, branchId, saleItems, user.id, `Sale ${sale.id}`, tx);
            ...
            await (0, idempotency_service_1.completeIdempotency)(tx, claim, [sale.id]);
            return sale;
        }, prisma_1.transactionOptions);
    },
    // ─── Add Payment to Existing Sale ─────────────────────────────────────────
    async addPayment(saleId, dto, user, idempotencyKey) {
        const storeId = (0, branch_access_1.requireStoreId)(user);
        return prisma_1.prisma.$transaction(async (tx) => {
            await (0, billing_state_service_1.assertStoreWritableInTransaction)(tx, storeId, "shared");
            const claim = await (0, idempotency_service_1.claimIdempotency)(tx, { storeId, userId: user.id, operation: "sale-payment" }, idempotencyKey, { saleId, dto, branchId: user.branchId });
            const currentSale = await sales_repository_1.SalesRepository.lockForPayment(saleId, storeId, tx);
            if (!currentSale)
                throw new AppError_1.AppError(404, "Sale not found");
            if ((0, role_access_1.isBranchScopedRole)(user.role) && currentSale.branchId !== user.branchId) {
                throw new AppError_1.AppError(403, "Forbidden");
            }
            if (claim?.replay)
                return sales_repository_1.SalesRepository.findById(saleId, storeId, tx);
            ...
            if (currentSale.customerId && debtReduced > 0) {
                await customers_repository_1.CustomersRepository.adjustBalance(currentSale.customerId, storeId, -debtReduced, tx);
            }
            await (0, idempotency_service_1.completeIdempotency)(tx, claim, [saleId]);
            return updated;
        }, prisma_1.transactionOptions);
    },
    // ─── Queries ──────────────────────────────────────────────────────────────
