...
- Transfer completion/cancellation atomically claims `PENDING` inside the transaction. A competing/repeated transition receives 409 and cannot move stock twice or overwrite a completed transfer with cancellation. Completion failure rolls the claim back. Existing branch permissions are retained.
...
| `POST /transfers` | Pending transfer creation |
...
3. Local image files currently require a consistent shared storage strategy across replicas. The deployment already includes Nginx; configure connection/body limits and upstream timeouts coherently with application settings. No new load balancer, microservices or Kubernetes layer is needed solely for this patch.
...
6. Review growth/retention of stock movements, audit records, receipts stored as database bytes, unused image files, auth handoffs and idempotency records. Indexes help lookup but do not replace retention/backup planning. Detailed sale payment histories and grouped reports retain existing shapes; add a client-coordinated paginated history/export API if these become large.
